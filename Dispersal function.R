...
dispersal <- function(maxDispersal,distDecay,dist_matrix,recruitment) # recruitment is named vector
{
  ...
  migrationNumer <- apply(dist_matrix,2,function(x){exp(-distDecay*x)}) # straying between sites, Anderson et al. 2015 EcoApps
  ...
  dispersers <- round(migration*recruitment)
