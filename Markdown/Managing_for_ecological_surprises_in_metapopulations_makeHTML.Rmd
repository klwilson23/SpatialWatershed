...
layout(matLayout)
linearSim <- metaPop(Npatches=16,networkType="linear",patchDistance=1,Nburnin=50,NyrsPost=50,omega=0.01,m=100,alpha=2,metaK=1600,cv=1e-6,DistScenario="uniform",magnitude_of_decline=0.9,lagTime=1,prodType="Beverton-Holt",rho.time=1e-5,rho.dist=1e5,compensationLag=25,dataWeighting=0.1,alphaVariable=FALSE,kVariable=FALSE,spatialPlots=FALSE)
...
seed <- 20190620
set.seed(seed)
trial <- metaPop(Npatches=16,networkType="complex",patchDistance=1,Nburnin=50,NyrsPost=50,omega=0.01,m=100,alpha=2,metaK=1600,cv=0.12,DistScenario="uniform",magnitude_of_decline=0.9,lagTime=1,prodType="Beverton-Holt",rho.time=1e-5,rho.dist=1e5,compensationLag=25,dataWeighting=0.1,alphaVariable=TRUE,kVariable=TRUE,spatialPlots=FALSE)
...
set.seed(seed)
trial <- metaPop(Npatches=16,networkType="complex",patchDistance=1,Nburnin=50,NyrsPost=50,omega=0.01,m=100,alpha=2,metaK=1600,cv=0.12,DistScenario="uniform",magnitude_of_decline=0.9,lagTime=1,prodType="Beverton-Holt",rho.time=1e-5,rho.dist=0.5,compensationLag=25,dataWeighting=0.1,alphaVariable=TRUE,kVariable=TRUE,spatialPlots=FALSE)
popDyn <- trial$popDyn
k_p <- trial$k_p
MetaPop <- trial$MetaPop
recovery <- trial$recovery
...
set.seed(seed)
trial <- metaPop(Npatches=16,networkType="complex",patchDistance=1,Nburnin=50,NyrsPost=50,omega=0.01,m=100,alpha=2,metaK=1600,cv=0.12,DistScenario="uniform",magnitude_of_decline=0.9,lagTime=1,prodType="Beverton-Holt",rho.time=0.75,rho.dist=1e5,compensationLag=25,dataWeighting=0.1,alphaVariable=TRUE,kVariable=TRUE,spatialPlots=FALSE)
popDyn <- trial$popDyn
k_p <- trial$k_p
MetaPop <- trial$MetaPop
recovery <- trial$recovery
...
We measured the following post-disturbance outcomes that have direction application for conservation metrics.
...
##### Proportion of simulations of unrecovered metapopulation for scenario with deterministic recruitment where patches are the same

We now show similar patterns in the lack of recovery across the metapopulation after $50$ generations post-disturbance.

```{r Recovery, echo=FALSE,fig.height=5.5,fig.width=6,fig.cap="Risk of unrecovered metapopulation after 50 generations along dispersal, disturbance (blue - uniform; light blue - localized, random; orange - localized, extirpation), and network gradients with high spatial-temporal correlation in recruitment variation.",fig.align="center"}
layout(matLayout)
par(mar=c(5,4,1,1))
dist_colours <- c("blue","dodgerblue","orange")
for(i in 1:length(scenarios$network))
{
  subResults <- results[results$network==scenarios$network[i] & 
                          ...
                          results$alpha==scenarios$alpha[1] & 
                          results$beta==scenarios$beta[1],]
  plot(subResults$dispersal,1-subResults$recovered,col=0,lty=0,type="b",lwd=0,pch=0,ylab="Risk of unrecovered metapopulation",xlab="Dispersal rate",ylim=1.7*range(c(0,1-results$recovered)))
  for(j in 1:length(scenarios$disturbance)){
    distResults <- subResults[subResults$disturbance==scenarios$disturbance[j],]
    lines(distResults$dispersal,1-distResults$recovered,col=dist_colours[j],type="b",pch=21,lwd=2,lty=1)
  }
}
...
```{r unrecovered with variable patches, echo=FALSE,fig.height=5.5,fig.width=6,fig.cap="Risk of unrecovered metapopulation after 50 generations along dispersal, disturbance (blue - uniform; light blue - localized, random; orange - localized, extirpation), and network gradients with variable patches.",fig.align="center"}
layout(matLayout)
par(mar=c(5,4,1,1))
dist_colours <- c("blue","dodgerblue","orange")
for(i in 1:length(scenarios$network))
...
```{r state shift with variable patches and stochasticity, echo=FALSE,fig.height=5.5,fig.width=6,fig.cap="Risk of unrecovered metapopulation after 50 generations along dispersal, disturbance (blue - uniform; light blue - localized, random; orange - localized, extirpation), and network gradients with variable patches and stochastic recruitment.",fig.align="center"}
layout(matLayout)
par(mar=c(5,4,1,1))
dist_colours <- c("blue","dodgerblue","orange")
for(i in 1:length(scenarios$network))
...
```{r state shifts with variable patches and space-time stochasticity, echo=FALSE,fig.height=5.5,fig.width=6,fig.cap="Risk of unrecovered metapopulation after 50 generations along dispersal, disturbance (blue - uniform; light blue - localized, random; orange - localized, extirpation), and network gradients with variable patches, and spatial-temporal correlations in stochastic recruitment.",fig.align="center"}
layout(matLayout)
par(mar=c(5,4,1,1))
dist_colours <- c("blue","dodgerblue","orange")
for(i in 1:length(scenarios$network))
