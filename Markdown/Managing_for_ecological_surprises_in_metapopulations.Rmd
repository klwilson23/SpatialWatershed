...
1. Recovered (a) & recovery rate (b) after disturbance:
    a. Recovered is defined as the number of simulations where metapopulation abundance averaged `1.0` of the average pre-disturbance abundance for `5` consecutive years post-disturbance. This "recovered rate" reflects the risk of a long-term state shift in metapopulation dynamics after disturbance in the face of stochasticity. 
...
We now show some general patterns in how variable patch demographic rates, network structure, dispersal, disturbance, recruitment stochasticity, and spatio-temporal correlations variation affects metapopulation *recovery rates*, *maximum sustainable yield* (analagous to the maximum rate of loss the system can sustain), and *recovered* (i.e, the number of simulations where the metapopulation fails to recover), and *patch occupancy* (i.e., number of patches with local abundance above some baseline).
...
for(i in 1:length(scenarios$network))
{
  ...
  plot(subResults$dispersal,subResults$recovery,col=0,lty=0,type="b",lwd=0,pch=0,ylab="Recovery (generations)",xlab="Dispersal rate",ylim=1.0*range(c(0,results$recovery)))
...
We now show similar patterns in the lack of recovery across the metapopulation after $100$ generations post-disturbance. Recovered % is defined as the number of simulations where metapopulation abundance averaged `1.0` of the average pre-disturbance abundance for `5` consecutive years after disturbance. This "recovered rate" reflects the risk of a long-term state shift in metapopulation dynamics after disturbance in the face of stochasticity. We start with a scenario with deterministic recruitment where patches are the same.

```{r Recovery, echo=FALSE,fig.height=5.5,fig.width=6,fig.cap="Risk of state shift after 100 generations along dispersal, disturbance (red - uniform; blue - localized, random; orange - localized, extirpation), and network gradients with high spatial-temporal correlation in recruitment variation.",fig.align="center"}
layout(matLayout)
par(mar=c(5,4,1,1))
for(i in 1:length(scenarios$network))
{
  subResults <- results[results$network==scenarios$network[i] & 
                          ...
                          results$alpha==scenarios$alpha[1] & 
                          results$beta==scenarios$beta[1],]
  plot(subResults$dispersal,subResults$recovered,col=0,lty=0,type="b",lwd=0,pch=0,ylab="Recovered (% of simulations)",xlab="Dispersal rate",ylim=1.75*range(c(0,results$recovered)))
  for(j in 1:length(scenarios$disturbance)){
    distResults <- subResults[subResults$disturbance==scenarios$disturbance[j],]
    lines(distResults$dispersal,distResults$recovered,col=dist_colours[j],type="b",pch=21,lwd=2,lty=1)
  }
}
...
```{r unrecovered with variable patches, echo=FALSE,fig.height=5.5,fig.width=6,fig.cap="Risk of state shift after 100 generations along dispersal, disturbance (red - uniform; blue - localized, random; orange - localized, extirpation), and network gradients with variable patches.",fig.align="center"}
layout(matLayout)
par(mar=c(5,4,1,1))
for(i in 1:length(scenarios$network))
{
