...
yieldRoot<-function(alpha,beta,Fcur,model)
{
  ...
  if(model=="Ricker")
  {
    ...
    yield <- ((-log(alpha)/beta)*(Ucur))-adults
  }
...
findMSY <- function(alpha,beta,Npatches,model){
  ...
  for(i in 1:Npatches){
    a_p <- alpha[i]
    b_p <- beta[i]
    F_msy[i] <- uniroot(fun_yield, interval=c(1e-5,1),alpha=a_p,beta=b_p, delta=0.0001,model=model)$root 
  }
  MSY <- sapply(1:Npatches,function(x){yieldRoot(alpha=alpha[x],beta=beta[x],Fcur=F_msy[[x]][1])})
...
model <- "Beverton-Holt"
findMSY(alpha=alpha,beta=beta,Npatches=Npatches,model="Beverton-Holt")
