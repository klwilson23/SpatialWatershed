...
ca <- ggplot(sub_ott,aes(x=Year_Seq,y=Abundance,fill=Region,colour=Region)) +
  ...
  theme_minimal() +
  xlab("") + ylab("") +
  annotate("text",label="(a)",x=38,y=2950,size=2.5)+
  scale_fill_manual(values="#d95f02") +
  scale_colour_manual(values="#d95f02") +
  theme(legend.position="none",panel.background = element_rect(fill=adjustcolor("white",0.90), colour="black"),panel.grid.major = element_blank(), panel.grid.minor = element_blank(),text=element_text(size=10),axis.text=element_text())
ca
library(cowplot)
